...
and convert_exp exp =
	...
	| GNU_BODY s ->
		Cxml.new_elt "body" [] (convert_block s)
...
and convert_stat stat =
	match stat with
	  NOP
	  -> Cxml.new_elt "nop" [] []
    | DEFINITION d -> Cxml.new_elt "def" [] (convert_def d)
	| COMPUTATION e
		-> convert_exp e
	| BLOCK stat
...
and convert_type _type =
    ...
    | SHARED _type -> Cxml.new_elt "shared" [] [convert_type _type]
    | GLOBAL _type -> Cxml.new_elt "global" [] [convert_type _type]
...
and convert_fundef _type store name body =

	let proto_elts = convert_proto _type in
	let body_elt = Cxml.new_elt "body" [] (convert_block body) in
...
and convert_def def =
	match def with
	  FUNDEF ((_, store, (name, _type, _, _)), body) ->
	  	[convert_fundef _type store name body]
	| OLDFUNDEF (_, _, _) ->
